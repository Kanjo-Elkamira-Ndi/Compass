...
| V16__add_extracted_text_to_research.sql | `research_analyses.extracted_text` column |
| **V17__create_lecturer_availability.sql** | Lecturer availability input for timetable generation |
| **V18__seed_demo_data.sql** | Fresh-dev-DB demo seed: 4 lecturers, 10 students, 10 courses (2025-2026 sem 1, with timetable slots + matching availability), 28 ENROLLED enrolments, 32 grade records + 16 performance summaries (2024-2025 history), 5 risk assessments, 3 career recommendations. Fixes the V10 admin password hash. Fixed UUIDs, so only apply on a fresh DB. |
| **V19__add_recommended_actions_to_risk_assessments.sql** | `risk_assessments.recommended_actions` column; backfills the V18 seed rows' malformed `risk_factors` JSON shape |
| **V20__create_complaint_module.sql** | Complaint portal: `complaints`, `complaint_replies`, `complaint_attachments`, `complaint_status_history` |
| **V21__create_notifications.sql** | In-app `notifications` table (complaint portal events, future modules) |
| **V22__seed_demo_accounts.sql** | Demo accounts for the login page (`ada@compass.edu` STUDENT, `dr.ngwa@compass.edu` LECTURER, password `password`) — idempotent, safe to re-run |
